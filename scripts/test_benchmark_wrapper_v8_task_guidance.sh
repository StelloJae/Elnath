...
required_current = [
    ...
    "if `nan_ok=True` or `rel` is supplied for datetime/timedelta",
    "test_datetime_nan_ok_raises",
    "test_timedelta_nan_ok_raises",
    "retained evidence showed recovery added tests first, hit `datetime.timedelta < int`, then stopped on budget before the tolerance patch",
    ...
    "channels: [false]",
    "complete standalone `test(\"...\", (t) => { ... });` block",
    "node --check test/get-release-to-add.test.js",
    ...
    "Keep the existing production diff intact, then finish the datetime/timedelta implementation before rerunning",
    "patch 'src/_pytest/python_api.py' so 'ApproxScalar.tolerance'",
    "ApproxScalar.tolerance' handles explicit 'datetime.timedelta' absolute tolerance before numeric '< 0' checks",
    ...
    "Do not remove or detach the existing 'test(\\\"Return versions merged between release branches\\\", (t) => {' block",
    "run 'node --check test/get-release-to-add.test.js' first",
    ...
    "Import/module-resolution fixes alone are incomplete",
    "the focused Mocha output must include the newly added cancellation regression",
    "replace runtime barrel-directory imports used by the focused spec with direct-file runtime imports",
    "without a `.ts` extension",
    "native strip-only TypeScript loading",
    "Do not change top-level imports or constructor parameter properties",
    "ts_bf002_loader_escape_churn",
    "task-specific recovery left TS-BF-002 import, public-option, existing-coverage, or focused-regression completion incomplete",
    "V8-MIX-BF-001 kustomize deterministic transformer guidance:",
    "Preserve resource order from `m.Resources()`",
    "Do not leave `go.work.sum` churn as the only non-production evidence",
    "final response self-reported incomplete regression evidence",
