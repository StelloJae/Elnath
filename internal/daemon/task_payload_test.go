...
func TestParseTaskPayloadPlainText(t *testing.T) {
	...
	if got.Type != TaskTypeAgent {
		t.Fatalf("Type = %q, want agent default", got.Type)
	}
...
func TestParseTaskPayloadLegacyJSONDefaultsToAgentType(t *testing.T) {
	got := ParseTaskPayload(`{"prompt":"hello"}`)
	if got.Type != TaskTypeAgent {
		t.Fatalf("Type = %q, want agent default", got.Type)
	}
}
...
func TestEncodeTaskPayloadRoundTripsResearchType(t *testing.T) {
	payload := TaskPayload{
		Type:      TaskTypeResearch,
		Prompt:    "ambient research loop",
		SessionID: "sess-123",
	}
...
func TestEncodeTaskPayloadKeepsResearchPromptOnlyStructured(t *testing.T) {
	payload := TaskPayload{Type: TaskTypeResearch, Prompt: "ambient research loop"}
	raw := EncodeTaskPayload(payload)
	if raw == payload.Prompt {
