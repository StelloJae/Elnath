...
func TestBuiltinToolMetadata(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:        "code_symbols document path",
			tool:        NewCodeSymbolsTool(guard),
			params:      rawJSON(`{"operation":"document_symbols","file_path":"foo.go"}`),
			wantSafe:    true,
			wantCancel:  false,
			wantReverse: true,
			wantScope:   ToolScope{ReadPaths: []string{guard.WorkDir()}},
		...
		{
			name:        "code_symbols workspace path",
			tool:        NewCodeSymbolsTool(guard),
			params:      rawJSON(`{"operation":"workspace_symbols","path":"subdir"}`),
			wantSafe:    true,
			wantCancel:  false,
			wantReverse: true,
			wantScope:   ToolScope{ReadPaths: []string{guard.WorkDir()}},
		},
		{
			...
			tool:        NewCodeSymbolsTool(guard),
			params:      rawJSON(`{"operation":"definition","file_path":"foo.go"}`),
			wantSafe:    true,
			wantCancel:  false,
			wantReverse: true,
			wantScope:   ConservativeScope(),
		...
		{
			name:        "code_symbols malformed json falls back",
			tool:        NewCodeSymbolsTool(guard),
			params:      json.RawMessage("{not valid"),
			wantSafe:    true,
			wantCancel:  false,
